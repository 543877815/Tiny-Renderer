...
void Shader::Use() // activate the shader
{
	glUseProgram(ID);
}

void Shader::CheckCompileErrors(const char* path, GLuint shader, std::string type)
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "PROGRAM")
	...
	else
	{
		...
		if (!success)
		{
			...
			std::cout << std::format("[{}] ERROR::PROGRAM_LINKING_ERROR of type: {} \n {} \n-- -------------------------------------------------- - -- ", path, type, infoLog) << std::endl;
		}
	}
}